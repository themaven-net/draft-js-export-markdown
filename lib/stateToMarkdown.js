...
var CODE_INDENT = '    ';
var LINE_BREAK = '  \n';
var STRIP_WHITESPACE = /([\s]*)([\W\w]*[^\s]+)([\s]*)/;
    ...
    value: function processBlock() {
      ...
      switch (blockType) {
        ...
        case _draftJsUtils.BLOCK_TYPE.BLOCKQUOTE:
          {
            this.insertLineBreaks(1);
            var blockquoted = this.renderBlockContent(block).split(/\r?\n/);
            for (var i = 0; i < blockquoted.length; i++) {
    ...
    value: function renderBlockContent(block) {
      ...
      return entityPieces.map(function (_ref) {
        ...
        var content = stylePieces.map(function (_ref3) {
          ...
          if (style.has(CODE)) {
            content = blockType === _draftJsUtils.BLOCK_TYPE.CODE ? content : content.replace(STRIP_WHITESPACE, '$1`$2`$3');
          }
          return content;
        }).join('');
        var entity = entityKey ? _draftJs.Entity.get(entityKey) : null;
        if (entity != null && entity.getType().toUpperCase() === _draftJsUtils.ENTITY_TYPE.LINK) {
          var data = entity.getData();
          var url = data.url || '';
          var title = data.title ? ' "' + escapeTitle(data.title) + '"' : '';
          return '[' + content + '](' + encodeURL(url) + title + ')';
        } else if (entity != null && entity.getType().toUpperCase() === _draftJsUtils.ENTITY_TYPE.IMAGE) {
